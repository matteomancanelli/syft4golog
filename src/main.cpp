...
int main(int argc, char ** argv) {
    ...
    CLI::Option* init_file_option =
        app.add_option("-i,--init-file", init_file, "Path to initial domain state file") ->
        required() -> check(CLI::ExistingFile);
    ...
    CLI::Option* dump_domain_dfa_option =
        app.add_option("-n,--dump-domain", dump_domain_dfa_flag, "Dump DFA of NDBAT domain as a collection of BDDs (1 = Yes; 0 = No. Default = 0)");
    
    CLI::Option* print_domain_flag_option =
        app.add_option("-p,--print-domain", print_domain_flag, "Print NDBAT domain (1 = Yes; 0 = No. Default = 0)");
    
    CLI::Option* dump_strategy_option =
        app.add_option("-s,--dump-strategy", dump_strategy_flag, "Dump winning strategy, if any, as a ShMTBDD (1 = Yes; 0 = No. Default = 0)");
    ...
    if (outfile != "") {
        ...
        if (!exists)
            outstream << "Domain File,Init File,Program File,Parse Domain (s),Parse Golog(s),Domain2DFA (s),Golog2TS (s),Game Construction (s),Game Resolution (s),Total (s),TS Size,Algorithm" << std::endl;
        const auto& runtimes = golog_synthesizer->running_times();
        const auto& rundata = golog_synthesizer->running_data();
        outstream << domain_file << "," << init_file << "," << golog_file << "," << runtimes[0] << "," << runtimes[1] << "," << runtimes[2] << "," << runtimes[3] << "," << runtimes[4] << "," << runtimes[5] << "," << sumVec(runtimes) << "," << static_cast<int>(rundata[0]); 
