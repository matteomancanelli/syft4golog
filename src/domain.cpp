...
void Domain::parse_sas() {
    ...
    Syft::SymbolicStateDfa Domain::to_ltlf_and_symbolic() {
        ...
        // LTLf driver
        std::shared_ptr<whitemech::lydia::parsers::ltlf::LTLfDriver> d =
            std::make_shared<whitemech::lydia::parsers::ltlf::LTLfDriver>();

        // parse formula
        std::stringstream formula_stream(pddl_ltlf);
        d->parse(formula_stream);
        auto result = d->get_result();

        // cast ast_ptr into ltlf_plus_ptr. Necessary since AbstractDriver is not template anymore
        auto ltlf_formula =
            std::static_pointer_cast<const whitemech::lydia::LTLfFormula>(result);  

        Syft::ExplicitStateDfa mona_dfa = Syft::ExplicitStateDfa::dfa_of_formula(*ltlf_formula);

        // debug
        mona_dfa.dfa_print();

        Syft::ExplicitStateDfaAdd explicit_dfa = Syft::ExplicitStateDfaAdd::from_dfa_mona(var_mgr_, mona_dfa);
        Syft::SymbolicStateDfa domain_dfa = Syft::SymbolicStateDfa::from_explicit(explicit_dfa);
    ...
    Syft::SymbolicStateDfa Domain::to_symbolic() {
        ...
        Syft::SymbolicStateDfa symbolic_dfa(var_mgr_, domain_dfa_id, dfa_initial_state, transition_function, final_states); 
    ...
    CUDD::BDD Domain::invariant_to_bdd(std::size_t automaton_id, const Invariant& inv) const {
        ...
        std::vector<CUDD::BDD> state_vars = var_mgr_->state_variables(automaton_id);
    ...
    void Domain::interactive(const Syft::SymbolicStateDfa& domain_dfa_) const {
