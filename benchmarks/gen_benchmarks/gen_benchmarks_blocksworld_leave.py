...
def gen_problem_bw(num_blocks):
    ...
    with open(f"./benchmarks/blocksworld_leave/p{num_blocks}.pddl", "w") as f:
        f.write(problem_str)

def write_program_exp1(num_blocks):
    unstack_some_block = "(" + "|".join([f"unstack_b{i}_b{i+1}" for i in range(1, num_blocks)]) + ")"
    put_down_some_block = "(" + "|".join([f"put_down_b{i}|put_down_succ_b{i}" for i in range(1, num_blocks + 1)]) + ")"
...
def write_program_exp2(num_blocks):
    ...
    put_down_succ_some_block = "(" + "|".join([f"put_down_succ_b{i}" for i in range(1, num_blocks + 1)]) + ")"
    loop_body = "(" + unstack_some_block + " ; (nil | " + put_down_some_block + ")" + " ; (nil | " + put_down_succ_some_block + "))*"
    final_condition = "[" + " && ".join([f"on_b{i}_b0" for i in range(1, num_blocks + 1)]) + "]?"
    return loop_body + " ; " + final_condition

def write_program_exp3(num_blocks):
    body = "(" + " ; ".join([f"(unstack_b{i}_b{i+1} ; (nil | put_down_b{i}); (nil | put_down_succ_b{i}))" for i in range(1, num_blocks)]) + ")"
    final_condition = "[" + " && ".join([f"on_b{i}_b0" for i in range(1, num_blocks + 1)]) + "]?"
...
def gen_program_bw(num_blocks, expertise):
    ...
    with open(f"./benchmarks/blocksworld_leave/prog_{expertise}_{num_blocks}.golog", "w") as f:
        f.write(program_str)
