...
RESULTS="$REPO_ROOT/results/results_dishwasher.csv"
...
for num_rooms in $(seq 1 5); do
  for num_dishes in $(seq 1 5); do
    DOMAIN="$REPO_ROOT/benchmarks/dishwasher/domain_r${num_rooms}.pddl"
    PROBLEM="$REPO_ROOT/benchmarks/dishwasher/p${num_rooms}_${num_dishes}.pddl"

    for expertise in 1 2 3; do
      GOLOG="$REPO_ROOT/benchmarks/dishwasher/prog_${expertise}_${num_rooms}_${num_dishes}.golog"
